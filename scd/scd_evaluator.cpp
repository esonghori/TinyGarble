...
int EvalauatePlaintext(const GarbledCircuit& garbled_circuit, const BIGNUM* p_init, const BIGNUM* g_init, const BIGNUM* e_init, const BIGNUM* p_input,
		const BIGNUM* g_input, const BIGNUM* e_input, uint64_t *clock_cycles, int64_t terminate_period, BIGNUM** outputs) {
	...
	for (uint64_t cid = 0; cid < (*clock_cycles); cid++) {  //for each clock cycle
			...
			for (uint64_t i = 0; i < garbled_circuit.dff_size; i++) {
				...
				if (wire_index == CONST_ZERO) {
					...
					if (wire_index < (int64_t) garbled_circuit.get_p_init_hi_index()) {
						b = BN_is_bit_set(p_init, wire_index - (int64_t) garbled_circuit.get_p_init_lo_index());
					} else if (wire_index < (int64_t) garbled_circuit.get_g_init_hi_index()) {
						b = BN_is_bit_set(g_init, wire_index - (int64_t) garbled_circuit.get_g_init_lo_index());
					} else {  // e_init
						b = BN_is_bit_set(e_init, wire_index - (int64_t) garbled_circuit.get_e_init_lo_index());
					}
				} else {
		...
		for (int64_t i = 0; i < (int64_t) garbled_circuit.get_input_size(); i++) {
			...
			if ((int64_t)(i + input_bias) < (int64_t) garbled_circuit.get_p_input_hi_index()) {  // input belongs to p
				bit_bias = cid * garbled_circuit.p_input_size + i + input_bias - (int64_t) garbled_circuit.get_p_input_lo_index();
				wires[input_bias + i] = BN_is_bit_set(p_input, bit_bias);

			} else if ((int64_t)(i + input_bias) < (int64_t) garbled_circuit.get_g_input_hi_index()) {  // input belongs to g
				bit_bias = cid * garbled_circuit.g_input_size + i + input_bias - (int64_t) garbled_circuit.get_g_input_lo_index();
				wires[input_bias + i] = BN_is_bit_set(g_input, bit_bias);
			} else {  // e
				bit_bias = cid * garbled_circuit.e_input_size + i + input_bias - (int64_t) garbled_circuit.get_e_input_lo_index();
		...
		if (terminate_period != 0 && garbled_circuit.terminate_id > 0) {
			if (cid % terminate_period == 0) {
				...
				if (is_terminate) {
					LOG(INFO) << "Terminated in " << cid + 1 << "cc out of " << *clock_cycles << "cc." << endl;
					*clock_cycles = cid + 1;
					break;
				}
			}
			...
			if (cid == (*clock_cycles) - 1) {
				LOG(ERROR) << "Not enough clock cycles. Circuit is not terminated in " << *clock_cycles << "cc." << endl;
			}
		}
	}
...
int EvalauatePlaintextStr(const string& scd_file_address, const string& p_init_str, const string& g_init_str, const string& e_init_str,
		const string& p_input_str, const string& g_input_str, const string& e_input_str, uint64_t clock_cycles, int64_t terminate_period,
	...
	LOG(INFO) << "p_init = " << BN_bn2hex(p_init) << "\tg_init = " << BN_bn2hex(g_init) << "\te_init = " << BN_bn2hex(e_init) << "\tp_input = "
			<< BN_bn2hex(p_input) << "\tg_input = " << BN_bn2hex(g_input) << "\te_input = " << BN_bn2hex(e_input) << endl;
	...
	EvalauatePlaintext(garbled_circuit, p_init, g_init, e_init, p_input, g_input, e_input, &clock_cycles, terminate_period, &outputs);
