...
int GarbleBNLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init, BIGNUM* p_input, BIGNUM* g_init, BIGNUM* g_input, uint64_t* clock_cycles,
		const string& output_mask, int64_t terminate_period, OutputMode output_mode, BIGNUM* output_bn, block R, block global_key, bool disable_OT,
		int connfd) {
	...
	CHECK(GarbleAllocLabels(garbled_circuit, &init_labels, &input_labels, &output_labels, &output_vals, R));

	CHECK(GarbleGneInitLabels(garbled_circuit, init_labels, R));

	uint64_t ot_start_time = RDTSC;
	{
		CHECK(GarbleTransferInitLabels(garbled_circuit, g_init, init_labels, disable_OT, connfd));
	}
	...
	for (uint64_t cid = 0; cid < (*clock_cycles); cid++) {
		...
		ot_start_time = RDTSC;
		{
			CHECK(GarbleTransferInputLabels(garbled_circuit, g_input, input_labels, cid, disable_OT, connfd));
		}
		ot_time += RDTSC - ot_start_time;

		garble_time += GarbleLowMem(garbled_circuit, p_init, p_input, init_labels, input_labels, garbled_tables_temp, garbled_tables, &garbled_table_ind, R,
				AES_Key, cid, connfd, wires, wires_val, dff_latch, dff_latch_val, fanout, &terminate_label, &terminate_val, output_labels, output_vals);
		...
		CHECK(GarbleTransferOutputLowMem(garbled_circuit, output_labels, output_vals, cid, output_mode, output_mask, output_bn, connfd));

		//if has terminate signal
		if (terminate_period != 0 && garbled_circuit.terminate_id > 0) {
			if (cid % terminate_period == 0) {
				bool is_terminate = false;
				CHECK(GarbleTransferTerminate(garbled_circuit, terminate_label, terminate_val, &is_terminate, connfd));
	...
	LOG(INFO) << "Alice transfer labels time (cc) = " << ot_time << "\t(cc/bit) = "
			<< ot_time / ((double) (garbled_circuit.e_init_size + (*clock_cycles) * garbled_circuit.e_input_size)) << endl;  //TODO:fix clock_cycle
...
int EvaluateBNLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init, BIGNUM* p_input, BIGNUM* e_init, BIGNUM* e_input, uint64_t* clock_cycles,
		const string& output_mask, int64_t terminate_period, OutputMode output_mode, BIGNUM* output_bn, block global_key, bool disable_OT, int connfd) {
	uint64_t ot_time = 0;
	...
	CHECK(EvaluateAllocLabels(garbled_circuit, &init_labels, &input_labels, &output_labels, &output_vals));
	uint64_t ot_start_time = RDTSC;
	{
		CHECK(EvaluateTransferInitLabels(garbled_circuit, e_init, init_labels, disable_OT, connfd));
	}
	ot_time += RDTSC - ot_start_time;
	...
	for (uint64_t cid = 0; cid < (*clock_cycles); cid++) {
		...
		ot_start_time = RDTSC;
		{
			CHECK(EvaluateTransferInputLabels(garbled_circuit, e_input, input_labels, cid, disable_OT, connfd));
		}
		ot_time += RDTSC - ot_start_time;
		...
		eval_time += EvaluateLowMem(garbled_circuit, p_init, p_input, init_labels, input_labels, garbled_tables, &garbled_table_ind, AES_Key, cid, connfd,
				wires, wires_val, dff_latch, dff_latch_val, fanout, &terminate_label, &terminate_val, output_labels, output_vals);
		...
		CHECK(EvaluateTransferOutputLowMem(garbled_circuit, output_labels, output_vals, cid, output_mode, output_mask, output_bn, connfd));

		//if has terminate signal
		if (terminate_period != 0 && garbled_circuit.terminate_id > 0) {
			if (cid % terminate_period == 0) {
				bool is_terminate = false;
				CHECK(EvaluateTransferTerminate(garbled_circuit, terminate_label, terminate_val, &is_terminate, connfd));
...
uint64_t GarbleLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init, BIGNUM* p_input, block* init_labels, block* input_labels,
		GarbledTable* garbled_tables_temp, GarbledTable* garbled_tables, uint64_t *garbled_table_ind, block R, AES_KEY& AES_Key, uint64_t cid, int connfd,
		BlockPair* wires, short* wires_val, BlockPair* dff_latch, short* dff_latch_val, int* fanout, BlockPair* terminate_label, short* terminate_val,
...
// table select
	...
	if (*garbled_table_ind != 0) {
		LOG(INFO) << "@" << cid << "\tgarbled_table_ind = " << *garbled_table_ind << endl;
	}

	return (end_time - start_time);
}

uint64_t EvaluateLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init, BIGNUM* p_input, block* init_labels, block* input_labels,
		GarbledTable* garbled_tables, uint64_t *garbled_table_ind, AES_KEY& AES_Key, uint64_t cid, int connfd, block *wires, short* wires_val, block *dff_latch,
		short* dff_latch_val, int* fanout, block* terminate_label, short* terminate_val, block* output_labels, short* output_vals) {
	*garbled_table_ind = 0;
	uint64_t start_time = RDTSC;
...
int GarbleAllocLabels(const GarbledCircuit& garbled_circuit, block** init_labels, block** input_labels, block** output_labels, short** output_vals, block R) {

// allocate and generate random init and inputs label pairs
	(*init_labels) = nullptr;
	if (garbled_circuit.get_init_size() > 0) {
		CHECK_ALLOC((*init_labels) = new block[garbled_circuit.get_secret_init_size() * 2]);
	...
	if (garbled_circuit.get_secret_input_size() > 0) {
		CHECK_ALLOC((*input_labels) = new block[garbled_circuit.get_secret_input_size() * 2]);
	}
...
int GarbleGneInitLabels(const GarbledCircuit& garbled_circuit, block* init_labels, block R) {
...
int GarbleGenInputLabels(const GarbledCircuit& garbled_circuit, block* input_labels, block R) {
	if (garbled_circuit.get_secret_input_size() > 0) {
...
int EvaluateAllocLabels(const GarbledCircuit& garbled_circuit, block** init_labels, block** input_labels, block** output_labels, short** output_vals) {

	(*init_labels) = nullptr;
	if (garbled_circuit.get_secret_init_size() > 0) {
		CHECK_ALLOC((*init_labels) = new block[garbled_circuit.get_secret_init_size()]);
	}

	(*input_labels) = nullptr;
	if (garbled_circuit.get_secret_input_size() > 0) {
		CHECK_ALLOC((*input_labels) = new block[garbled_circuit.get_secret_input_size()]);
	}
...
int GarbleOTInitLowMem(const GarbledCircuit& garbled_circuit, block* init_labels, int connfd) {
...
int GarbleOTInputLowMem(const GarbledCircuit& garbled_circuit, block* input_labels, uint64_t cid, int connfd) {
...
int EvalauteOTInitLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* e_init, block* init_labels, int connfd) {
	uint32_t message_len = garbled_circuit.e_init_size;
	if (message_len == 0) {
		return SUCCESS;
	}
...
int EvalauteOTInputLowMem(const GarbledCircuit& garbled_circuit, BIGNUM* e_input, block* input_labels, uint64_t cid, int connfd) {
	uint32_t message_len = garbled_circuit.e_input_size;
	if (message_len == 0) {
		return SUCCESS;
	}
...
int GarbleTransferInitLabels(const GarbledCircuit& garbled_circuit, BIGNUM* g_init, block* init_labels, bool disable_OT, int connfd) {
...
// e_init
		...
		for (uint i = 0; i < garbled_circuit.e_init_size; i++) {
			if (i >= (uint) BN_num_bits(e_init) || BN_is_bit_set(e_init, i) == 0) {
				CHECK(SendData(connfd, &init_labels[(i + garbled_circuit.g_init_size) * 2 + 0], sizeof(block)));
			} else {
				CHECK(SendData(connfd, &init_labels[(i + garbled_circuit.g_init_size) * 2 + 1], sizeof(block)));
			}
		}
		BN_free(e_init);
...
int GarbleTransferInputLabels(const GarbledCircuit& garbled_circuit, BIGNUM* g_input, block* input_labels, uint64_t cid, bool disable_OT, int connfd) {
...
// e_input
		...
		for (uint i = 0; i < garbled_circuit.e_input_size; i++) {
			if (cid * garbled_circuit.e_input_size + i >= (uint) BN_num_bits(e_input) || BN_is_bit_set(e_input, cid * garbled_circuit.e_input_size + i) == 0) {
				CHECK(SendData(connfd, &input_labels[(i + garbled_circuit.g_input_size) * 2 + 0], sizeof(block)));
			} else {
				CHECK(SendData(connfd, &input_labels[(i + garbled_circuit.g_input_size) * 2 + 1], sizeof(block)));
			}
		}
...
int EvaluateTransferInitLabels(const GarbledCircuit& garbled_circuit, BIGNUM* e_init, block* init_labels, bool disable_OT, int connfd) {
// g_init
	for (uint i = 0; i < garbled_circuit.g_init_size; i++) {
...
int EvaluateTransferInputLabels(const GarbledCircuit& garbled_circuit, BIGNUM* e_input, block* input_labels, uint64_t cid, bool disable_OT, int connfd) {
// g_input
	for (uint i = 0; i < garbled_circuit.g_input_size; i++) {
...
// e_input
		...
		for (uint i = 0; i < garbled_circuit.e_input_size; i++) {
			CHECK(RecvData(connfd, &input_labels[i + garbled_circuit.g_input_size], sizeof(block)));
		}
	} else {
		CHECK(EvalauteOTInputLowMem(garbled_circuit, e_input, input_labels, cid, connfd));
	}
	return SUCCESS;
}

int GarbleTransferOutputLowMem(const GarbledCircuit& garbled_circuit, block* output_labels, short* output_vals, uint64_t cid, OutputMode output_mode,
		const string& output_mask, BIGNUM* output_bn, int connfd) {
...
int EvaluateTransferOutputLowMem(const GarbledCircuit& garbled_circuit, block* output_labels, short* output_vals, uint64_t cid, OutputMode output_mode,
		const string& output_mask, BIGNUM* output_bn, int connfd) {
	BIGNUM* output_mask_bn = BN_new();
	BN_hex2bn(&output_mask_bn, output_mask.c_str());
