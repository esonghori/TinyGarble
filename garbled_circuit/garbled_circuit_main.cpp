...
int CheckOptionsAlice(const string& file_address, uint64_t clock_cycles, const string& output_mask, bool disable_OT, bool low_mem_foot, int connfd) {
...
int CheckOptionsBob(const string& file_address, uint64_t clock_cycles, const string& output_mask, bool disable_OT, bool low_mem_foot, int connfd) {
	...
	int status;
	if (file_address_ != file_address || clock_cycles_ != clock_cycles || output_mask_ != output_mask || disable_OT_ != disable_OT
...
int main(int argc, char* argv[]) {
	...
	boost::format fmter("Garble and Evaluate Netlist, TinyGarble version %1%.%2%.%3%.\nAllowed options");
	fmter % TinyGarble_VERSION_MAJOR % TinyGarble_VERSION_MINOR % TinyGarble_VERSION_PATCH;
	po::options_description desc(fmter.str());
	desc.add_options()  //
	("help,h", "produce help message")  //
	("alice,a", "Run as Alice (server).")  //
	("bob,b", "Run as Bob (client).")  //
	("scd_file,i", po::value < string > (&file_address)->default_value(string(TINYGARBLE_BINARY_DIR) + "/scd/netlists/hamming_32bit_1cc.scd"),
			"Simple circuit description (.scd) file address.")  //
	("port,p", po::value<int>(&port)->default_value(1234), "socket port")  //
	("server_ip,s", po::value < string > (&server_ip)->default_value("127.0.0.1"), "Server's (Alice's) IP, required when running as Bob.")  //
	("p_init", po::value < string > (&p_init_f_hex_str)->default_value("0"), "File or Hexadecimal public init for initializing DFFs. In case of file,"
			" each line should contain multiple of 4 bits (e.g., 4bit, 8bit, 32bit).")  //
	("p_input", po::value < string > (&p_input_f_hex_str)->default_value("0"), "File or Hexadecimal public input.")  //
	("init", po::value < string > (&init_f_hex_str)->default_value("0"), "Hexadecimal init for initializing DFFs.")  //
	("input", po::value < string > (&input_f_hex_str)->default_value("0"), "File or Hexadecimal input.")  //
	("clock_cycles,c", po::value < uint64_t > (&clock_cycles)->default_value(1), "Number of clock cycles to evaluate the circuit.")  //
	("dump_directory", po::value < string > (&dump_prefix)->default_value(""), "Directory for dumping memory hex files.")  //
	...
	("output_mask", po::value < string > (&output_mask)->default_value("0"), "Hexadecimal mask for output. 0 indicates that output belongs to Bob, "
	...
	if (vm.count("output_mode")) {
		if (vm["output_mode"].as<string>() == "0" || vm["output_mode"].as<string>() == "consecutive") {
			output_mode = OutputMode::consecutive;
		} else if (vm["output_mode"].as<string>() == "1" || vm["output_mode"].as<string>() == "separated_clock") {
			output_mode = OutputMode::separated_clock;
		} else if (vm["output_mode"].as<string>() == "2" || vm["output_mode"].as<string>() == "last_clock") {
			output_mode = OutputMode::last_clock;
		} else {
			LOG(ERROR) << "ERROR: output_mode should be in "
					"{0:consecutive, 1:separated_clock, 2:last_clock}" << endl;
			std::cout << desc << endl;
			return FAILURE;
		}
	}
	...
	if (vm.count("disable_OT")) {
		disable_OT = true;
		LOG(INFO) << "OT is disabled. WARNING:OT is crucial for GC security." << endl;
	}
...
//		LOG(INFO) << "Measured BW (Gbps) = " << ((float) n * s * 256 / timeS / (1000000000.0)) << endl;
		...
		if (vm.count("server_ip")) {
			...
			LOG(ERROR) << "Server IP should be specified, when running as Bob." << endl << endl;
			std::cout << desc << endl;
			return -1;
		}
		...
		LOG(INFO) << "Connect Bob's client to Alice's server on " << server_ip << ":" << port << endl;
