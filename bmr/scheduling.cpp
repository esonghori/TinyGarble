...
int TopSort(const vector<ReadBMRGate>& G, int64_t no_task, int64_t *index) {
...
int DFS(const ReadBMRCircuit &read_circuit, vector<int> *status, int wire_index,
        vector<int> *loop) {

  const vector<ReadBMRGate>& Gates = read_circuit.gate_list;
...
int FindPath(const ReadBMRCircuit &read_circuit, vector<int> *loop) {
...
int Schedule(const ReadBMRCircuit &read_circuit, int64_t no_core, int64_t **core) {

  const vector<ReadBMRGate>& G = read_circuit.gate_list;
  int64_t no_task;
...
int TopologicalSortMultiCore(ReadBMRCircuit &read_circuit) {
...
int TopologicalSortVisit(const ReadBMRCircuit &read_circuit, vector<Mark>* marks,
