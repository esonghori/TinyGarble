...
void LogInitial(int argc, char *argv[]) {
	...
	if (log_2_std || error_2_std) {
		...
		string file_addresss_error = file_addresss_prefix + who + "-error.log";
		log_map[ERROR] = new std::ofstream(file_addresss_error.c_str(), std::ios::out);
	}
}
...
ostream& LogStream(int log_code) {
	if (log_code != ERROR && log_code != INFO) {
		std::cerr << "No such a LOG stream. Use LOG(INFO) or LOG(ERROR)" << log_code << endl;
		return std::cerr;
	}
...
std::ostream & operator <<(std::ostream & o, const block& v) {
	...
	o << std::hex << std::setfill('0') << std::setw(8) << v_u32[3] << " " << std::setfill('0') << std::setw(8) << v_u32[2] << " " << std::setfill('0')
			<< std::setw(8) << v_u32[1] << " " << std::setfill('0') << std::setw(8) << v_u32[0];
	o.flags(f);
	return o;
}
