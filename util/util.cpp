 ...
 * offset R.
 *
 * These values must be unpredictable to the other party: R plus the input
 * labels is enough to recover the garbler's private inputs. So this has to be a
 * cryptographic generator, not a fast one. It is called once per input wire per
 * clock cycle, not once per gate, so OpenSSL's buffered CSPRNG is not on the
 * hot path.
 *
 * Aborts rather than returning a predictable block if the CSPRNG fails; the
 * return type leaves nowhere to report an error, and continuing with weak
 * labels would silently void the protocol's security.
 */
block RandomBlock() {
  block b;
  if (RAND_bytes((unsigned char *) &b, sizeof(block)) != 1) {
    LOG(ERROR) << "RAND_bytes failed: no secure randomness available, aborting "
               "rather than emitting predictable wire labels."
               << endl;
    exit(EXIT_FAILURE);
  }
  return b;
